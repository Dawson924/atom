...
export class IPCService {
    ...
    register<T = any>(
        ...
        const wrappedHandler = this.createWrappedHandler(handler, options?.errorHandling ?? true);
    ...
    handle<T = any>(channel: string, handler: HandlerFunction<T>) {
        return this.register('handle', channel, handler);
    }
    ...
    private createWrappedHandler<T>(
        ...
        return async (event: any, ...args: any[]) => {
            try {
                ...
                console.error(`IPC Error in channel ${event.channel}:`, error);
