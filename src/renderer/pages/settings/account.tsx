...
export default function AccountPage() {
    ...
    useEffect(() => {
        if (!session || !session.signedIn || session.mode === 'offline') return;
    ...
    const handleChange = async (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        await ConfigService.set(e.target.name, e.target.value);
        await fetchData();
    };
            ...
            .then((result) => {
                if (!result.hasMultipleProfiles) {
                    ...
                    openModal(SelectPromptModal, {
                        ...
                        message: 'Found multiple available profiles',
                        items: result.availableProfiles.map((profile: any) => {
                            return {
                                label: profile.name,
                                value: profile.id,
                                onSelect: () => UserService.setUser({
                                    profile: profile.id
                                }).then(refreshSession)
                            };
                        })
                        ...
                        <>
                            ...
                            <div className="px-2 my-2 space-x-4 w-full flex justify-end">
                                ...
                                <Button
                                    ...
                                    variant="outlined"
                                    color="error"
                                    disabled={!session.profile && true}
                                    onClick={() => {
                                        UserService.deleteProfile(session.profile.id)
                                            .then(fetchData);
                                    }}
                                >
                                    Remove
